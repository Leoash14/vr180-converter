...
def simple_vr180_conversion(input_path):
    """Simplified VR180 conversion that works on Railway"""
    try:
        print(f"[INFO] Starting simple VR180 conversion for: {input_path}")
        ...
        # Extract frames (limit to first 100 frames for Railway)
        while count < min(100, frame_count):
            ret, frame = cap.read()
            if not ret:
                break
        ...
        for frame in frames:
            ...
            left_frames.append(left_padded)
            
            # Right eye (shifted left)
            right_crop = frame[:, :-stereo_offset]
            right_padded = np.pad(right_crop, ((0, 0), (0, stereo_offset), (0, 0)), mode='edge')
            right_frames.append(right_padded)
        
        # Create output video using OpenCV (no FFmpeg dependency)
        output_path = "vr180_output.mp4"
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(output_path, fourcc, fps, (1280, 360))
        ...
        print(f"[INFO] VR180 conversion completed: {output_path}")
