...
# Try to import vr180_converter, fallback to mock if not available
try:
    from vr180_converter import convert_to_vr180
    CONVERSION_AVAILABLE = True
except ImportError:
    CONVERSION_AVAILABLE = False
    st.warning("⚠️ VR180 conversion module not available. Using demo mode.")
...
def init_db():
    try:
        # Use temporary database for cloud deployment
...
def register_user(email, password, db_path):
    try:
        conn = sqlite3.connect(db_path, timeout=10)
        c = conn.cursor()
...
def create_demo_user(db_path):
    try:
        if not login_user("demo@vr180.com", "demo123", db_path):
            register_user("demo@vr180.com", "demo123", db_path)
    except Exception as e:
...
def mock_convert_to_vr180(input_path):
    """Mock conversion for demo purposes"""
    import shutil
    output_path = "vr180_demo_output.mp4"
    # Copy input to output as demo
...
if not st.session_state.authenticated:
    ...
    with tab1:
        with st.form("login_form"):
            ...
            if login_submitted:
                if email and password and db_path:
                    if login_user(email, password, db_path):
    ...
    with tab2:
        with st.form("register_form"):
            ...
            if register_submitted:
                if new_email and new_password and db_path:
                    if register_user(new_email, new_password, db_path):
    ...
    if st.session_state.current_state == "upload":
        ...
        try:
            if st.session_state.uploaded_file:
                ...
                # Convert (use mock if real conversion not available)
                if CONVERSION_AVAILABLE:
                    output_path = convert_to_vr180(f"uploads/{st.session_state.uploaded_file.name}")
                else:
                    output_path = mock_convert_to_vr180(f"uploads/{st.session_state.uploaded_file.name}")
